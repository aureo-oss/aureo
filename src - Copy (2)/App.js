...
function AppContent() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'how-to-use': return <ErrorBoundary><HowToUse /></ErrorBoundary>;
      default: return (
        <div className="dashboard-grid">
          <div className="welcome-card bg-gray-800 border border-gray-700">
            <h2 className="text-white">Welcome back, {currentUser?.displayName || 'Event Manager'}!</h2>
            <p className="text-gray-300">Manage your events efficiently with our comprehensive tools</p>
          </div>
          
          <div className="quick-actions-container bg-gray-800 border border-gray-700">
            <div className="quick-actions">
              <h3 className="text-gray-300">Quick Actions</h3>
              <div className="action-buttons">
                <button 
                  onClick={() => setActiveTab('projects')}
                  className="action-button bg-gray-700 hover:bg-gray-600 text-white"
                >
                  Create New Event
                </button>
                <button 
          ...
          <div className="dashboard-feature-container bg-gray-800 border border-gray-700">
            <div className="dashboard-feature-content">
              <div className="dashboard-feature-text">
                <h3 className="text-white">Event Management Made Simple</h3>
              ...
              >
                ...
                >
                  {currentUser?.photoURL ? (
                    ...
                    <span className="profile-icon-fallback bg-gray-700">
                      {currentUser?.displayName?.charAt(0).toUpperCase() || 'U'}
                    </span>
                  )}
                </button>
                {showUserDropdown && (
                  <div className="dropdown-menu bg-gray-800 border border-gray-700">
                    <button 
                      onClick={() => {
                        setActiveTab('account');
                        setShowUserDropdown(false);
                      }}
                      className="dropdown-item text-white hover:bg-gray-700 border-b border-gray-700"
                    >
                      Account Settings
                    </button>
                    <button 
        ...
        <Route path="*" element={
          currentUser ? (
            <div className={`app-main ${isSidebarOpen ? 'sidebar-open' : ''}`}>
              <aside 
                className={`app-sidebar ${isSidebarOpen ? 'open' : ''} bg-gray-900 border-r border-gray-700 ${isMobile ? 'mobile-sidebar' : ''}`}
                style={{
                  transform: isMobile ? 
                ...
                <nav className="sidebar-nav">
                  <ul>
                    {[
                      ...
                      { id: 'how-to-use', icon: 'M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22ZM12 16V12M12 8H12.01', text: 'How to Use' }
                    ].map((item) => (
                      <li 
                        key={item.id} 
                        className={`nav-item ${activeTab === item.id ? 'active bg-gray-700' : ''} hover:bg-gray-800 text-gray-300`}
              ...
              <main 
                className="main-content bg-gray-800"
                style={{
